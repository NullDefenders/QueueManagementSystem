...
import type { TalonInQueue, WindowCall, NewTalonMessage, CallMessage, WindowStatusMessage, EventMessage } from '../types';

const EventMonitor: React.FC = () => {
    const [queueMap, setQueueMap] = useState<Map<string, TalonInQueue>>(() => new Map());

    const [callsMap, setCallsMap] = useState<Map<string, WindowCall>>(() => new Map());

    const [freeWindows, setFreeWindows] = useState<Set<string>>(() => new Set());

    useEffect(() => {
        const eventSource = new EventSource('http://localhost:5122/api/events');
    ...
    const handleNewTalon = (newTalon: NewTalonMessage) => {
        ...
        const talonValue: TalonInQueue = { TalonNumber: talonKey, PendingTime: newTalon.PendingTime };
    ...
    const handleTalonCall = (callData: CallMessage) => {
        ...
        const callValue: WindowCall = { TalonNumber: talonKey, WindowNumber: windowKey };
    ...
    return (
        <div style={{ display: 'flex', justifyContent: 'space-around', padding: '20px' }}>
            ...
            <div style={{ width: '45%', borderRight: '1px solid #ccc', paddingRight: '20px' }}>

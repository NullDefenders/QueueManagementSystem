...
public class RedisQueueStorage : IQueueStorage, IAsyncDisposable
{
  private readonly ILogger<RedisQueueStorage> _logger;
  private readonly IConfiguration _configuration;
  private readonly IConnectionMultiplexer _redisConnection;
  public IDatabase _redisdb;

  public RedisQueueStorage(ILogger<RedisQueueStorage> logger, IConfiguration configuration, MessageProcessorFactory processorFactory)
  {
    _logger = logger;
  ...
  public Task AddWindowAsync(string windowNumber)
    =>AddIfNotExistsAsync("WINDOWS", windowNumber);

  public async Task<string?> GetWindowAsync()
  {
    return (await _redisdb.ListRightPopAsync("WINDOWS")).ToString();
  }


  public Task RemoveWindowAsync(string windowNumber)
    =>_redisdb.ListRemoveAsync("WINDOWS", windowNumber);
